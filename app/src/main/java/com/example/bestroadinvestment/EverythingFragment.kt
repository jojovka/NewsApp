...
class EverythingFragment : Fragment(), EverythingNewsFethcer, SearchViewOnChangeListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fetchNewsItems(page)
        val favouritePageButton: Button = requireActivity().findViewById(R.id.fav_button)
        favouritePageButton.setOnClickListener {
    ...
    private fun initializeRecyclerView() {
        recyclerView = requireActivity().findViewById(R.id.recycler_view_everything)
    ...
    private fun fetchNewsItems(page: Int = 0) {
        isLoading = true
        val task = EverythingFetchingAsyncTask("", page, this)

        task.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR)
    }

    override fun whenEverythingNewsFetchedSuccessfully(articles: List<Article>?) {
