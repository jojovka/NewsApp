...
interface NewsFetchedListener {
    ...
    fun whenHeadlinesNewsFetchedSuccessfully(articles: List<Article>?)
...
@Suppress("DEPRECATION")
class NewsFetchingAsyncTask(
        private val q: String? = null,
        private val page: Int = 0,
        private val newsFetchedListener: HeadlinesFragment? = null
    ...
    override fun doInBackground(vararg p0: String?) {
        val myurl =
                "https://newsapi.org/v2/top-headlines?q=covid&apiKey=e65ee0938a2a43ebb15923b48faed18d&page=$page"
        this.sendGet(myurl)
    }
    ...
    private fun parseReturnedJsonData(s: String) {
        ...
        if (rt.status == "ok") {
            Handler(Looper.getMainLooper()).post {
                newsFetchedListener?.whenHeadlinesNewsFetchedSuccessfully(rt.articles)
            }
        } else {
            Handler(Looper.getMainLooper()).post {
